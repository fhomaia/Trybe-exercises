...
const calculateOriginFactor = (country) => {
  country === 'Brazil' ?  8 : 7;
}

const calculateWaterFrequency = (needsSun, origin, size) => {
  if(needsSun) return size *  0.77 + calculateOriginFactor(origin);
  return (size / 2) *  1.33 + calculateOriginFactor(origin);
}

const initPlant = (id, breed, needsSun, origin, specialCare, size) => {
  const waterFrequency = calculateWaterFrequency(needsSun, origin, size);
  const newPlant = {
    id,
    breed,
    needsSun,
...
const savePlants = (defaultPlants) => {
